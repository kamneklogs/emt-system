...
public class UserRoleRepositoryImpl implements UserRoleRepository {
    ...
    private static final String SELECT_USER_ROLE_TABLE_COLUMNS = USERNAME + ", " + ID;
    private static final String INSERT_USER_ROLE_TABLE_COLUMNS = USERNAME + ", " + ID;

    private static final String SELECT_FROM_USER_ROLE = "SELECT " + SELECT_USER_ROLE_TABLE_COLUMNS + " FROM "
            + USER_ROLE_FULL_TABLE_NAME;
    private static final String SELECT_FROM_USER_ROLE_WHERE_USERNAME = SELECT_FROM_USER_ROLE + " WHERE " + USERNAME
            + " = ?";
    ...
    public void save(User user, Role role) {
        jdbcTemplate.update(INSERT_INTO_USER_ROL, new Object[] { user.getUsername(), role.getId() });
    }

    @Override
    public boolean userHasRole(User user, Role role) {
        return jdbcTemplate.queryForRowSet(SELECT_FROM_USER_ROLE_WHERE_USERNAME_AND_ID,
                new Object[] { user.getUsername(), role.getId() }).first();
    }

    @Override
    public List<Integer> findUserRoleIdsByUsername(String username) {
        try {
    ...
    public void deleteUserRoleByUsernameAndRoleId(User user, Role role) {
        jdbcTemplate.update(DELETE_FROM_USER_ROLE_WHERE_USERNAME_AND_ID,
                new Object[] { user.getUsername(), role.getId() });
    }
}
