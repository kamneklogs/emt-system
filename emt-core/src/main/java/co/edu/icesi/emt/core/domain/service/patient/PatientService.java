...
public class PatientService {
    ...
    public Patient findById(String id) {
        ...
        if (patient != null) {
            patient.setPersonalInformation(
                    this.personalInformationService.findById(patient.getId()));
        }
        return patient;
    }
    ...
    @Transactional
    public void save(String patientId, PersonalInformation personalInformation) {
        if (this.personalInformationService.existsById(patientId)) {
            this.personalInformationService.update(personalInformation);
        } else {
            this.personalInformationService.save(personalInformation);
        }

        this.patientRepository.save(new Patient(patientId, Instant.now()));
    }
}
